  ...
  createBinlogStream(opts, cb) {
    ...
    function createStream() {
      ...
      const stream = new Readable({
        objectMode: true,
        read() {
          if (started) return conn.resume()
          conn._registerSlave(opts, () => {
            const dumpCmd = conn._binlogDump(opts);
            dumpCmd.on('event', (ev) => {
              if (!this.push(ev)) conn.pause()
            });
            dumpCmd.on('eof', () => {
              this.push(null);
            ...
            conn.on('error', e => this.destroy(e))
          });
          started = true
        },
      });
    ...
    conn.query("show global variables like 'binlog_checksum'", (e, rs, fs) => {
      if (e) return cb(e)
      opts.verifyChecksum = rs.length > 0 && rs[0][fs[1].name] !== 'NONE'
      if (opts.verifyChecksum)
        conn.query('set @master_binlog_checksum = @@global.binlog_checksum', (e) => {
          if (e) return cb(e)
          createStream()
        })
      else
