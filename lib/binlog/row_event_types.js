...
class RowsEvent extends BinLogEvent {
  ...
  _read_values_name(
    ...
    switch (column.type) {
      ...
      case Types.GEOMETRY:
        return this.packet.readBuffer(this.packet.readIntBySize(column.length_size));
      case Types.JSON:
  ...
  _read_string(size, column) {
    ...
    const encoding = column.character_set_name != null ? this.charset_to_encoding(column.character_set_name) : 'utf8'
  ...
  _read_new_decimal(column) {
    ...
    // FIXME
    this.packet.skip(compressedBytes[compressedIntegral] + uncompressedIntegral + uncompressedFractional + compressedBytes[compressedFractional])
    return '0'

    // Read the sign byte
    let value = this.packet.readInt8();
    let res = '';
