...
class Column {
  ...
  _parseColumnDefinition(packet) {
    ...
    switch (this.type) {
      ...
      case Types.BIT:
        ...
        const bits_val = packet.readUInt8();
        const bytes_val = packet.readUInt8();
  ...
  _readStringMetadata(packet) {
    ...
    const metadata = packet.readUInt16LE();
    const real_type = metadata >> 8; // The high byte determines the effective type.
    ...
    if (real_type === Types.SET || real_type === Types.ENUM) {
      ...
      this.type = real_type;
      this.size = metadata & 0x00FF; // The low byte is the size (1 or 2 bytes).
    } else {
      ...
      this.max_length = (((metadata >> 4) & 0x300) ^ 0x300) + (metadata & 0x00FF);
    }
  }
  ...
  get data() {
    ...
    for (const key in this) {
      ...
      if (Object.prototype.hasOwnProperty.call(this, key) && typeof this[key] !== 'function' && !key.startsWith('_')) {
