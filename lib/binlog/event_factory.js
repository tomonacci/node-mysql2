...
const Packet = require('./Packet.js'); // Import Packet class
const { BinlogEventHeader, calculateCrc32 } = require('./binlog-event-header.js'); // Import header and CRC32 helper
const BinLogEvent = require('./binlog-event-base.js'); // Import base BinLogEvent
...
const {
  ...
  PreviousGtidsEvent,
  NotImplementedEvent,
} = require('./binlog-event-types.js');

// Import all specific MariaDB event types
const MariadbEventTypes = require('./mariadb_event_types.js');

// Import all specific Row event types
const RowEventTypes = require('./row_event_types.js'); // Contains RowsEvent, DeleteRowsEvent, WriteRowsEvent, UpdateRowsEvent, PartialUpdateRowsEvent

// Import TableMapEvent
const { TableMapEvent } = require('./table_event_types.js');

const Types = require('./constants/types.js'); // Assuming this contains event type constants
...
const ALL_EVENT_CLASSES = {
  ...
  [Types.GTID_EVENT]: GtidEvent,
  [Types.PREVIOUS_GTIDS_EVENT]: PreviousGtidsEvent,
  ...
  [Types.MARIADB_BINLOG_CHECKPOINT_EVENT]: MariadbEventTypes.MariadbBinLogCheckPointEvent,
  [Types.MARIADB_ANNOTATE_ROWS_EVENT]: MariadbEventTypes.MariadbAnnotateRowsEvent,
  [Types.MARIADB_GTID_LIST_EVENT]: MariadbEventTypes.MariadbGtidListEvent,
  [Types.MARIADB_START_ENCRYPTION_EVENT]: MariadbEventTypes.MariadbStartEncryptionEvent,
};
...
class BinlogEventFactory {
  ...
  static fromPacket(packet, tableMap, ctlConnection, options = {}) {
    ...
    if (options.verifyChecksum) {
      ...
      // The data for CRC starts at `binlogEventStartOffset` and ends at `binlogEventStartOffset + eventHeader.eventSize`.
      const crcDataStart = binlogEventStartOffset;
      const crcDataEnd = crcDataStart + eventHeader.eventSize; // Total event size from header

      // Extract the buffer segment for CRC32 calculation
      const dataForCrc = packet.buffer.slice(crcDataStart, crcDataEnd);

      // The 4-byte footer (checksum itself) is immediately after the event data.
      const footerStart = crcDataEnd;
      const footerEnd = footerStart + 4;

      // Ensure we don't try to read beyond the available buffer.
      if (footerEnd > packet.end) { // packet.end is the actual end of the packet's readable buffer
          throw new Error("Cannot read checksum: packet buffer ends unexpectedly before checksum.");
      }
      const footer = packet.buffer.slice(footerStart, footerEnd);

      const calculatedCrc = calculateCrc32(dataForCrc);
      const calculatedCrcBuffer = new Uint32Array([calculatedCrc]); // Use Uint32Array to mimic buffer for comparison
      const footerUint32 = new Uint32Array(footer.buffer, footer.byteOffset, 1);

      if (calculatedCrcBuffer[0] !== footerUint32[0]) {
        console.error(
          `CRC32 checksum failed for event type ${eventHeader.eventType} (0x${eventHeader.eventType.toString(16)}). ` +
          `Calculated: 0x${calculatedCrc.toString(16)}, Expected: 0x${footerUint32[0].toString(16)}.`
        );
        throw new Error(`CRC32 checksum failed for event type ${eventHeader.eventType}`);
      }
    }

    // Determine the correct subclass based on eventType.
    // Use the ALL_EVENT_CLASSES object for direct lookup.
    const EventClass = ALL_EVENT_CLASSES[eventHeader.eventType] || NotImplementedEvent;
