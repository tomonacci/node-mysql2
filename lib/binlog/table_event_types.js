// table-event-types.js
// Implements the MySQL TableMapEvent.

'use strict';

const BinLogEvent = require('./binlog-event-base.js');
const Column = require('./column.js'); // Import mock Column class
const Table = require('./table.js');   // Import mock Table class
const { charset_by_id } = require('./constants/CHARSET.js'); // Import charset helper
const MetadataFieldType = require('./metadata-field-type.js'); // Import MetadataFieldType enum
const Types = require('./constants/types.js'); // Import FIELD_TYPE constants
...
class TableMapEvent extends BinLogEvent {
  ...
  constructor(fromPacket, eventHeader, tableMap, ctlConnection, options = {}) {
    ...
    if (this.tableId in this.tableMap && this.__freeze_schema) {
      this.processed = false;
      return;
    }

    this.flags = this.packet.readUInt16LE();

    // Payload
    this.schemaLength = this.packet.readUInt8(); // !B
    this.schema = this.packet.readString(this.schemaLength, 'utf8');
    this.packet.skip(1); // Skip null byte
    this.tableLength = this.packet.readUInt8(); // !B
    ...
    this.packet.skip(1); // Skip null byte
    this.columnCount = this.packet.readLengthCodedBinary();

    this.columns = [];
    // Assuming dbms is available from ctlConnection or is defaulted
    this.dbms = this.dbms || (this.ctlConnection && this.ctlConnection._get_dbms ? this.ctlConnection._get_dbms() : 'mysql');

    // Read column types
    const columnTypes = this.packet.readBuffer(this.columnCount);
    this.packet.readLengthCodedBinary(); // Read metadata block length (ignored here as we read directly)

    // Parse column metadata based on types and actual metadata block
    // This is a simplified `Column` instantiation. In a real scenario, the Column
    // constructor would take `fromPacket` and parse the column-specific metadata.
    for (let i = 0; i < columnTypes.length; i++) {
      const columnType = columnTypes[i];
      // Here, Column needs to be able to read its own specific metadata if `fromPacket`
      // is truly used, or TableMapEvent should pass detailed metadata.
      // For this mock, Column initializes basic type.
      const col = new Column(columnType, fromPacket); // Pass packet to Column for its own parsing
    ...
    this.tableObj = new Table(this.tableId, this.schema, this.table, this.columns);
    this.tableMap[this.tableId] = this.tableObj;
  ...
  _get_optional_meta_data() {
    ...
    // Ensure we don't read past the event's payload, considering the checksum.
    // this.eventSize is total event size. 19 is common header. 4 is checksum.
    const payloadEndOffset = this.eventSize - 19 - Types.BINLOG.BINLOG_CHECKSUM_LEN;
    ...
    while (this.packet.offset < startOfOptionalMetadata + payloadEndOffset) {
      if (this.packet.offset + 1 > this.packet.length) { // Ensure enough bytes for type
        break; // Reached end of packet prematurely
      }
      const optionMetadataType = this.packet.readUInt8(); // Read 1 byte for type

      if (this.packet.offset + 1 > this.packet.length) { // Ensure enough bytes for length-coded binary length
        break;
      }
      const length = this.packet.readLengthCodedBinary(); // Read length-coded binary length
      ...
      if (dataEndOffset > this.packet.length) {
          console.warn(`Optional metadata field type ${optionMetadataType} declared length ${length} but only ${this.packet.length - this.packet.offset} bytes remaining.`);
          this.packet.skip(this.packet.length - this.packet.offset); // Skip remaining bytes to avoid error
          break;
      }

      switch (fieldType) {
        case MetadataFieldType.SIGNEDNESS:
          optionalMetadata.unsigned_column_list = this._convert_include_non_numeric_column(
            this._read_bool_list(length, true)
          );
          break;
        case MetadataFieldType.DEFAULT_CHARSET:
          const [defaultCharsetCollation, charsetCollation] = this._read_default_charset(length);
          optionalMetadata.default_charset_collation = defaultCharsetCollation;
          optionalMetadata.charset_collation = charsetCollation;
          optionalMetadata.charset_collation_list = this._parsed_column_charset_by_default_charset(
        ...
        case MetadataFieldType.COLUMN_CHARSET:
          ...
          optionalMetadata.charset_collation_list = this._parsed_column_charset_by_column_charset(
        ...
        case MetadataFieldType.ENUM_STR_VALUE:
          optionalMetadata.set_enum_str_value_list = this._read_type_values(length);
          break;
        ...
        case MetadataFieldType.PRIMARY_KEY_WITH_PREFIX:
          optionalMetadata.primary_keys_with_prefix = this._read_primary_keys_with_prefix(length);
          break;
        case MetadataFieldType.ENUM_AND_SET_DEFAULT_CHARSET:
          const [enumSetDefaultCharset, enumSetCharsetCollation] = this._read_default_charset(length);
          optionalMetadata.enum_and_set_default_charset = enumSetDefaultCharset;
          optionalMetadata.enum_and_set_charset_collation = enumSetCharsetCollation;
          optionalMetadata.enum_and_set_collation_list = this._parsed_column_charset_by_default_charset(
        ...
        case MetadataFieldType.ENUM_AND_SET_COLUMN_CHARSET:
          optionalMetadata.enum_and_set_default_column_charset_list = this._read_ints(length);
          optionalMetadata.enum_and_set_collation_list = this._parsed_column_charset_by_column_charset(
            optionalMetadata.enum_and_set_default_column_charset_list,
            this._is_enum_or_set_column
          );
          break;
        case MetadataFieldType.VISIBILITY:
          optionalMetadata.visibility_list = this._read_bool_list(length, false);
          break;
        case MetadataFieldType.UNKNOWN_METADATA_FIELD_TYPE:
        default:
          console.warn(`Unknown optional metadata field type: 0x${optionMetadataType.toString(16)} with length ${length}. Skipping ${length} bytes.`);
    ...
    for (let columnIdx = 0; columnIdx < this.columnCount; columnIdx++) {
      ...
      if (this._is_character_column(columnType, this.dbms)) {
        if (charsetPos < this.optionalMetadata.charset_collation_list.length) {
          const charsetId = this.optionalMetadata.charset_collation_list[charsetPos];
          charsetPos++;
      ...
      if (this._is_enum_or_set_column(columnType, this.dbms)) {
        if (enumOrSetPos < this.optionalMetadata.enum_and_set_collation_list.length) {
          const charsetId = this.optionalMetadata.enum_and_set_collation_list[enumOrSetPos];
          enumOrSetPos++;
          const charsetInfo = charset_by_id(charsetId, this.dbms);
          if (charsetInfo) {
            columnData.collation_name = charsetInfo.collation;
        ...
        if (this._is_enum_column(columnType)) {
          if (enumPos < this.optionalMetadata.set_enum_str_value_list.length) {
            const enumColumnInfo = this.optionalMetadata.set_enum_str_value_list[enumPos];
        ...
        if (this._is_set_column(columnType)) {
          if (setPos < this.optionalMetadata.set_str_value_list.length) {
            const setColumnInfo = this.optionalMetadata.set_str_value_list[setPos];
      ...
      if (this.optionalMetadata.unsigned_column_list && columnIdx < this.optionalMetadata.unsigned_column_list.length) {
      ...
      if (this.optionalMetadata.simple_primary_key_list && this.optionalMetadata.simple_primary_key_list.includes(columnIdx)) {
      ...
      // Set visibility flag
      if (this.optionalMetadata.visibility_list && columnIdx < this.optionalMetadata.visibility_list.length) {
   ...
   * @private
   */
  _parsed_column_charset_by_column_charset(columnCharsetList, columnTypeDetectFunction) {
    const columnCharset = [];
    let position = 0;
  ...
  _read_default_charset(length) {
    ...
    const defaultCharsetCollation = this.packet.readLengthCodedBinary();
    while (this.packet.offset < readUntilOffset) {
      const columnIndex = this.packet.readLengthCodedBinary();
      const charsetCollation = this.packet.readLengthCodedBinary();
  ...
  _read_ints(length) {
    ...
    const readUntilOffset = this.packet.offset + length;
    while (this.packet.offset < readUntilOffset) {
      result.push(this.packet.readLengthCodedBinary());
    }
    return result;
  }
  ...
  _read_column_names(length) {
    ...
    while (this.packet.offset < readUntilOffset) {
      ...
      try {
        ...
        console.warn(`Failed to decode column name: ${e.message}. Using hex representation.`);
  ...
  _read_type_values(length) {
    ...
    while (this.packet.offset < readUntilOffset) {
      const typeValueList = [];
      const valueCount = this.packet.readLengthCodedBinary();
      for (let i = 0; i < valueCount; i++) {
        ...
        try {
          ...
          console.warn(`Failed to decode type value string: ${e.message}. Skipping.`);
  ...
  _is_character_column(columnType, dbms = 'mysql') {
    if ([
      ...
      Types.FIELD_TYPE.VAR_STRING,
      Types.FIELD_TYPE.VARCHAR,
      Types.FIELD_TYPE.BLOB,
    ].includes(columnType)) {
      return true;
    }
    // MariaDB can treat GEOMETRY as character column in some contexts
    if (columnType === Types.FIELD_TYPE.GEOMETRY && dbms === 'mariadb') {
      return true;
    }
    return false;
  }
  ...
  _is_enum_column(columnType) {
    return columnType === Types.FIELD_TYPE.ENUM;
  }
  ...
  _is_set_column(columnType) {
    return columnType === Types.FIELD_TYPE.SET;
  }
  ...
  _is_enum_or_set_column(columnType, dbms = 'mysql') {
    return [Types.FIELD_TYPE.ENUM, Types.FIELD_TYPE.SET].includes(columnType);
  }
  ...
  _is_numeric_column(columnType) {
    return [
      Types.FIELD_TYPE.TINY,
      Types.FIELD_TYPE.SHORT,
      Types.FIELD_TYPE.INT24,
      Types.FIELD_TYPE.LONG,
      Types.FIELD_TYPE.LONGLONG,
      Types.FIELD_TYPE.NEWDECIMAL,
      Types.FIELD_TYPE.FLOAT,
      Types.FIELD_TYPE.DOUBLE,
      Types.FIELD_TYPE.YEAR,
    ].includes(columnType);
  }
}
...
class OptionalMetaData {
  ...
  dump() {
    ...
    console.log(`unsigned_column_list: ${JSON.stringify(this.unsigned_column_list)}`);
    ...
    console.log(`set_str_value_list : ${JSON.stringify(this.set_str_value_list)}`);
    console.log(`set_enum_str_value_list : ${JSON.stringify(this.set_enum_str_value_list)}`);
    console.log(`geometry_type_list : ${JSON.stringify(this.geometry_type_list)}`);
    console.log(`simple_primary_key_list: ${JSON.stringify(this.simple_primary_key_list)}`);
    console.log(`primary_keys_with_prefix: ${JSON.stringify(this.primary_keys_with_prefix)}`);
    console.log(`visibility_list: ${JSON.stringify(this.visibility_list)}`);
    console.log(`charset_collation_list: ${JSON.stringify(this.charset_collation_list)}`);
    console.log(`enum_and_set_collation_list: ${JSON.stringify(this.enum_and_set_collation_list)}`);
  }
}
