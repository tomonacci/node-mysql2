...
class TableMapEvent extends BinLogEvent {
  ...
  constructor(fromPacket, eventHeader, tableMap, ctlConnection, options = {}) {
    ...
    if (this.packet.offset !== metadataStartOffset + metadataLength) {
      console.warn(`column data offset mismatch for table ${this.table}: got ${this.packet.offset}, expected ${metadataStartOffset + metadataLength}`)
  ...
  _get_optional_meta_data() {
    ...
    while (this.packet.haveMoreData()) {
      if (this.packet.numRemainingBytes() < 1) { // Ensure enough bytes for type
        break; // Reached end of packet prematurely
      }
      const optionMetadataType = this.packet.readInt8(); // Read 1 byte for type

      if (this.packet.numRemainingBytes() < 1) { // Ensure enough bytes for length-coded binary length
        break;
      }
      ...
      if (this.packet.numRemainingBytes() < length) {
          console.warn(`Optional metadata field type ${optionMetadataType} declared length ${length} but only ${this.packet.numRemainingBytes()} bytes remaining.`);
      ...
      if (this.packet.offset !== dataEndOffset) {
          ...
          console.warn('dataEndOffset mismatch', this.packet.offset, dataEndOffset)
