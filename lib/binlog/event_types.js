...
const Types = require('../constants/query_event_status.js'); // Assuming this contains relevant event type constants
...
class FormatDescriptionEvent extends BinLogEvent {
  ...
  constructor(fromPacket, eventHeader, tableMap, ctlConnection, options = {}) {
    ...
    this.mysqlVersionStr = this.packet
      .readBuffer(50)
      .toString('utf8')
      .replace(/\0+$/, '');
    ...
    this.postHeaderLen = Array.from(this.packet.readBuffer()); // Corresponds to Python's struct.unpack(f"<{n}B", self.packet.read(n))
...
class QueryEvent extends BinLogEvent {
  ...
  constructor(fromPacket, eventHeader, tableMap, ctlConnection, options = {}) {
    ...
    if (this.packet.offset > statusVarsEndPos) {
      console.warn(
        `buffer overrun when parsing status variables: this.offset ${this.packet.offset}, statusVarsEndPos ${statusVarsEndPos}`
      );
      this.packet.offset = statusVarsEndPos;
    }
    //*/
    //this.statusVars = parseStatusVars(this.packet.readBuffer(this.statusVarsLength))

    this.schema = this.packet.readString(this.schemaLength);
    this.packet.skip(1); // Skip the null byte after schema

    this.query = this.packet.readString();
  }
    ...
    switch (key) {
      ...
      case Types.Q_UPDATED_DB_NAMES: // 0x0C
        ...
        for (let i = 0; i < mtsAccessedDbs; i++) {
          const dbName = this.packet.readNullTerminatedString('utf8');
          dbs.push(dbName);
        }
      ...
      case Types.Q_XID:
        ...
        break;
      default:
        throw new StatusVariableMismatch(
          `Unknown status variable key: 0x${key.toString(16)}`
        );
    }
  }
}
...
class UserVarEvent extends BinLogEvent {
  ...
  _readInt(buffer, flags) {
    ...
    console.warn(
      `UserVarEvent._readInt: Unexpected buffer length for integer: ${buffer.length}`
    );
  ...
  _readDecimal(buffer) {
    ...
    if (this.tempValueBuffer.length < 2) {
      console.warn(
        'UserVarEvent._readDecimal: tempValueBuffer too short for precision/decimals.'
      );
