...
class Packet {
  ...
  readIntBySize(size) {
    if (size === 8) return this.readInt64JSNumber()
    let r = 0
    for (let i = 0; i < size; i++) {
      r |= this.buffer[this.offset + i] << (8 * i)
    }
    this.offset += size;
    return r
  }

  readIntBeBySize(size) {
    if (size === 8) return (this.buffer.readInt32BE(this.offset) << 32) | this.buffer.readInt32BE(this.offset + 4)
    let r = 0
    for (let i = 0; i < size; i++) {
      r <<= 8
      r |= this.buffer[this.offset + i]
    }
    this.offset += size
    return r
  }
  ...
  // FIXME
  readSInt24() {
    return this.readInt16() | (this.readSInt8() << 16)
  }
  ...
  readNullTerminatedString(encoding) {
    ...
    let end = this.offset;
    // FIXME: while (this.buffer[end] && end < this.end) {
  ...
  writeString(s, encoding) {
    ...
    // this.offset += bytes;
    const buf = StringParser.encode(s, encoding ?? 'utf8');
    this.buffer.length && buf.copy(this.buffer, this.offset);
    this.offset += buf.length;
  }
  ...
  writeLengthCodedNumber(n) {
    ...
    //this.buffer.writeUInt32LE(Math.floor(n / 0x100000000), this.offset); // FIXME: Corrected for writing 64-bit
    this.offset += 4;
    return this.offset;
  }
